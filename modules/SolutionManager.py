    ...
    def check_box(possible_value_, solution_):
        ...
        for i in [1, 4, 7]:
            for j in [1, 4, 7]:
                ...
                for k in SolutionManager.box_range(i):
                    for l in SolutionManager.box_range(j):
                ...
                possible_element = [x for b in [value for key, value in possible_value_.items()
                                                if key[0] in SolutionManager.box_range(i) and key[1] in SolutionManager.box_range(j) and len(value) > 0]
                                    for x in b]
                unique = [x for x in possible_element if possible_element.count(x) == 1]
                if len(unique) > 0:
                    for k in unique:
                        for key, value in {key: value for key, value in possible_value_.items()
                                        if key[0] in SolutionManager.box_range(i) and key[1] in SolutionManager.box_range(j) and len(value) > 0}.items():
                            if k in value:
...
def loop_basic_rule(possible_value_, solution_):
    ...
    while True:
        ...
        SolutionManager.check_row(possible_value_, solution_)
        SolutionManager.check_column(possible_value_, solution_)
        SolutionManager.check_box(possible_value_, solution_)
        SolutionManager.check_unique_possible_value(possible_value_, solution_)
    ...
    for i in [1, 4, 7]:
        for j in [1, 4, 7]:
            possible_list = {key: value for key, value in possible_value_.items() if
                             key[0] in SolutionManager.box_range(i) and key[1] in SolutionManager.box_range(j) and len(value) > 0}
            algorithm(possible_list, possible_value_, solution_)
    return 0
        ...
        for j in [1, 4, 7]:
            ...
            for x in possible_element:
                available_cell = [key for key, value in possible_value_.items()
                                  if x in value if key[0] in SolutionManager.box_range(i) and key[1] in SolutionManager.box_range(j)]
